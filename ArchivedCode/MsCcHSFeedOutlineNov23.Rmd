...
# Leaf matter consumed vs. max mass
...
```{r}
# OLD DON'T RUN
#newarea <- ggplot(aes(x = area, y = mass.max, color = food, shape = fate), 
       data = datameans) + 
  theme_bw() + 
  ylim(0, 20) + 
  xlim(0, 6000) +
...
# Violin plots of max mass

```{r}
data$treatment <- factor(data$treatment, levels = c("NP NHS", "NP HS", "P NHS", "P HS"))

violins <- ggplot(data = data, aes(x = treatment, y = mass.max)) + 
...
ggsave("~/Desktop/violins.png", violins, width = 8, height = 5)
```
