...
```{r}
violins <- ggplot(data = dataviolin, aes(x = treatment, y = mass.max/1000)) + 
  geom_violin(fill = "light grey", color = "light grey") + 
  geom_jitter(aes(color = fate, shape = food), width = 0.1, height = 0) + 
  theme_bw() + 
  labs(x = "Treatment Group", y  = "Maximum mass achieved (g)",  color = "System Outcome", shape = "Hostplant") + 
  scale_color_manual(values = c("wander" = "#1E88E5" , "em" = "lime green", 
                                       "wowe" = "orange", "wowe mongo" =  "#D81B60"),
                    labels = c("Caterpillar pre-pupation", "Wasp emergence", 
                               "Wasp larval death", "Wasp egg death")) +
...
barsdead <- ggplot(data = datadead, aes(x = treatment, fill = fate)) + 
  ...
  labs(x = "Treatment group", y = "Proportion of indiviudals", fill = "System Outcome") + 
  scale_fill_manual(values = c("dead" = "purple", "wander" = "#1E88E5" , "em" = "lime green", 
                                       "wowe" = "orange", "wowe mongo" =  "#D81B60"),
                    labels = c("Died before expt. end", "Caterpillar pre-pupation", "Wasp emergence", 
                               "Wasp larval death", "Wasp egg death")) + 
...
```{r}
datalong5$treatment <- factor(datalong5$treatment, labels = c("Non-parasitized non-heat shocked", "Non-parasitized heat shocked", "Parasitized non-heat shocked", "Parasitized heat shocked"))
...
newmeansfacet <- ggplot(data = datalong5, aes(x = ttmax, y = mass.max, color = fate, shape = food)) + 
  ...
  scale_shape_manual(values = c(1, 19), labels = c("Devil's claw", "Tobacco")) + 
  scale_color_manual(values = c("#1E88E5", "lime green", "orange", "#D81B60"), 
                     labels = c("Caterpillar pre-pupation", "Wasp emergence", "Wasp larval death", 
                                "Wasp egg death")) + 
#  geom_point(aes(x = ttmax, y = mass.max, color = fate, shape = food), data = datameans2, size = 4) + 
#  geom_errorbar(data = datameans2, aes(ymin = mass.max - semass, ymax = mass.max + semass), size = .5) + 
#  geom_errorbarh(data = datameans2, aes(xmin = ttmax - se, xmax = ttmax + se), size = .5) + 
#  guides(size = FALSE) + 
  labs(x = "Age at maximum mass (days)", y = "Maximum mass achieved (g)", 
       shape = "Hostplant", color = "System Outcome") 
...
newarea <- ggplot(data = datalong5, aes(x = area, y = mass.max, color = fate, shape = food)) + 
  theme_bw() + 
  geom_point(alpha = 0.4) + 
  scale_shape_manual(values = c(1, 19), labels = c("Devil's claw", "Tobacaco")) + 
  scale_color_manual(values = c("#1E88E5", "lime green", "orange", "#D81B60"), 
                     labels = c("Caterpillar pre-pupation", "Wasp emergence", "Wasp larval death", 
                                "Wasp egg death")) + 
  ylim(0, 20) + 
