...
# Fig 2: Wasp survival across plant types 
...
ggsave("~/Desktop/GitHub/MsCcHSFeed/Figs/newwaspplot.png", newwaspplot, width = 8, height = 5)
```



```{r}
waspplot <- ggplot(data = datawasp, aes(x = food, y = percent)) +
  theme_bw() + 
  geom_boxplot() + 
  geom_jitter(aes(color = treatment), width = 0.1, height = 0) + 
  labs(x = "Hostplant", y = "Percent survival from emergence to eclosion", color = "Treatment")

waspplot

ggsave("~/Desktop/GitHub/MsCcHSFeed/Figs/waspplot.png", plot = waspplot, width = 8, height = 5)
...
```{r}
waspbinom <- glm(percent ~ food, family = "binomial", weights = num.w.tot, data = datawasp)
...
```{r}
waspbinom2 <- glm(percent ~ food, family = "binomial", weights = num.w.tot, 
                  data = datawasp[datawasp$treatment == "P NHS", ])

summary(waspbinom2)

anova(waspbinom2, test = "Chisq")

```

```{r}
waspemerge <- lm(num.w.tot ~ food, data = datawasp)
...
```{r}
waspemergetreat <- lm(num.w.tot ~ treatment, data = datawasp)

summary(waspemergetreat)
```


# Fig 3: Violin plots of max mass
...
ggsave("~/Desktop/GitHub/MsCcHSFeed/Figs/violins2.png", violins, width = 8, height = 5)

```

# Fig 4: Developmental outcome proportions
...
ggsave("~/Desktop/GitHub/MsCcHSFeed/Figs/outcomesdead.png", plot = barsdead, width = 8, height = 5)
```

# Fig 5: Maximum mass and age
...
ggsave("~/Desktop/GitHub/MsCcHSFeed/Figs/newmeans.png", newmeans, width = 8, height = 5)
```
...
# Fig 6: Leaf matter consumed vs. max mass
...
ggsave("~/Desktop/GitHub/MsCcHSFeed/Figs/newarea.png", newarea, width = 8, height = 5)
```
...
```{r}
binomWOWE <- data[data$treatment == "P HS", ] %>%
  select(ID, food, fate)
...
```{r}
binomWE <- data[data$treatment == "P HS", ] %>%
  select(ID, food, fate)
